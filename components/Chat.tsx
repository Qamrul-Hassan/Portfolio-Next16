...
const Chat = () => {
  ...
  return (
    ...
    <div className="fixed bottom-4 right-4 z-9999 sm:w-80 w-64 min-w-[16rem] pointer-events-none"> {/* Added pointer-events-none */}
      ...
      {isOpen && (
        ...
        <div className="absolute bottom-full right-0 mb-2 bg-[#CECECE] rounded-lg shadow-xl sm:w-80 w-64 min-w-[16rem] flex flex-col max-h-[70vh] pointer-events-auto"> {/* Added flex, max-h, pointer-events-auto */}
          {/* Header */}
          <header className="flex justify-between items-center bg-pink-500 text-white py-3 px-4 rounded-t-lg shrink-0"> {/* Added flex-shrink-0 */}
                ...
                >
                  ...
                  >
                    ...
                    {msg.sender === 'user' && index === messages.length - (messages[messages.length-1]?.sender === 'bot' ? 2 : 1) && editingMessageIndex === null && (
                      <button
                        className="absolute -bottom-1 -left-1 p-1 text-[10px] text-blue-600 bg-white rounded-full shadow opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity duration-200" // Improved visibility
