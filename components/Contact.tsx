...
const Contact = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    emailjs.send(serviceID, ownerTemplateID, templateParams, publicKey)
      .then(() => emailjs.send(serviceID, userTemplateID, templateParams, publicKey))
      .then(() => {
  ...
  return (
    <section className="bg-[#CECECE] py-20 px-6 lg:px-16 text-white">
      <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2 }}>
        <h2 className="text-4xl md:text-5xl font-bold mb-4">
          <span className="text-pink-500">Get</span> <span className="text-gray-800"> In Touch</span>
        </h2>
        <motion.p className="text-lg md:text-2xl text-gray-800" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5, delay: 0.5 }}>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center bg-[#434343] py-16 px-8 rounded-lg shadow-lg space-y-8 lg:space-y-0 lg:space-x-4">
        <motion.form onSubmit={handleSubmit} className="w-full lg:w-2/4 bg-[#b5b5b5] p-8 rounded-lg shadow-xl" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.5 }}>
          <div className="mb-6">
