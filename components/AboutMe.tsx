...
const AboutMe = () => {
  ...
  const handleRequestResume = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/cv/request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) });
      const data = (await response.json()) as { requestId?: string; error?: string; expiresInSeconds?: number; delivery?: { accepted?: string[] } };
      if (!response.ok || !data.requestId) { setRequestState("error"); setRequestMessage(data.error || "Could not send verification code."); return; }
      ...
      setRequestMessage(acceptedTo ? `Verification code sent to ${acceptedTo}. Expires in ${minutes} minutes.` : `Verification code sent. Expires in ${minutes} minutes.`);
  ...
  const handleVerifyOtp = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setRequestState("loading"); setRequestMessage("");
    try {
      const response = await fetch("/api/cv/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ requestId, otp }) });
      const data = (await response.json()) as { cvUrl?: string; error?: string };
      if (!response.ok || !data.cvUrl) { setRequestState("error"); setRequestMessage(data.error || "Invalid or expired code."); return; }
  ...
  return (
    <section id="about" className="relative overflow-hidden py-20 px-6 lg:px-16 text-gray-100"
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 items-start">
        {/* Left: text + resume */}
        <motion.div className="lg:w-1/2" initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <motion.span className="inline-block mb-3 px-3 py-1 rounded-full text-xs font-bold tracking-[0.2em] uppercase"
          ...
          <h2 className="text-5xl md:text-6xl font-black mb-4 text-white" style={{ fontFamily: "'Playfair Display', serif" }}>
            About{" "}
          ...
          <p className="text-lg leading-relaxed mb-5 text-gray-300">
            I am{" "}<span className="font-semibold text-sky-600">Qamrul Hassan</span>, a Web developer with a passion for crafting visually appealing and responsive web interfaces. With hands-on experience in modern tools and technologies, I specialize in transforming ideas into functional web solutions. I have a strong foundation in frontend development, focusing on creating user-friendly designs and maintaining performance.
          </p>
          ...
          <motion.button type="button"
            onClick={() => { setShowResumeForm(p => !p); setRequestState("idle"); setRequestMessage(""); setOtp(""); setRequestId(""); setOtpSent(false); setCvUrl(""); }}
          ...
          {showResumeForm && (
            <motion.form onSubmit={otpSent ? handleVerifyOtp : handleRequestResume}
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {otpSent && (<button type="button" onClick={() => { setOtpSent(false); setOtp(""); setRequestId(""); setCvUrl(""); setRequestState("idle"); setRequestMessage(""); }}
                  className="inline-flex items-center rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Change Email</button>)}
                <button type="button" onClick={() => { setShowResumeForm(false); setOtp(""); setRequestId(""); setOtpSent(false); setCvUrl(""); }}
                  className="inline-flex items-center rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Close</button>
              </div>
              {cvUrl && (<a href={cvUrl} target="_blank" rel="noopener noreferrer"
                className="mt-3 inline-flex w-fit items-center rounded-lg border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-bold text-sky-700 hover:bg-sky-100">Open CV</a>)}
              {requestMessage && (<p className={`mt-3 text-sm ${requestState === "error" ? "text-red-600" : "text-emerald-600"}`}>{requestMessage}</p>)}
            </motion.form>
          )}
        </motion.div>

        {/* Right: Skills grid */}
        <motion.div className="lg:w-1/2 grid grid-cols-2 md:grid-cols-3 gap-4"
          ...
          {skills.map(({ Icon, label, color }, index) => {
            ...
            return (
              <motion.div key={index}
                ...
                <div className="relative z-10 h-9 w-9 rounded-full flex items-center justify-center ring-1 ring-sky-300/30 group-hover:ring-sky-400/50 transition"
